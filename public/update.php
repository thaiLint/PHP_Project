...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $product = array_merge($product, compact('name','sku','price','stock','status','image_url'));
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("UPDATE products SET name=?, sku=?, price=?, stock=?, status=?, image_url=? WHERE id=?");
        $stmt->execute([$name, $sku, $price, $stock, $status, $image_url, $id]);
...
<title>Shodai — Edit Product</title>
...
.page-title{font-family:'Playfair Display',serif;font-size:38px;font-weight:900;letter-spacing:-1px;line-height:1.05;}
.page-title span{color:var(--accent)}
.page-sub{font-size:14px;color:var(--text2);margin-top:8px}
.id-badge{
  background:var(--surface2);border:1px solid var(--border);
  border-radius:20px;padding:6px 14px;
  font-size:12px;font-weight:700;color:var(--text3);white-space:nowrap;margin-top:8px;
}
...
label{display:block;font-size:12px;font-weight:700;letter-spacing:.8px;text-transform:uppercase;color:var(--text2);margin-bottom:8px;}
...
input:focus,select:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(240,192,64,.12);}
...
.btn-submit{
  flex:1;padding:15px;background:var(--accent);color:#0d0d14;
...
.btn-discard{
  padding:15px 22px;background:var(--surface2);color:var(--text2);
  font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;
  border:1px solid var(--border);border-radius:var(--radius-sm);cursor:pointer;
  text-decoration:none;transition:all .2s;display:flex;align-items:center;gap:7px;
}
.btn-discard:hover{border-color:var(--text2);color:var(--text)}
</style>
</head>
<body>
...
<div class="page-wrap">
  <div class="page-header">
    <div>
      <h1 class="page-title">Edit<br><span>Product</span></h1>
      <p class="page-sub">Update the details for this product.</p>
    </div>
  ...
  <div class="form-card">
    <form method="POST" action="" novalidate>
      ...
      <div class="form-group">
        ...
        <input type="text" name="name" placeholder="Product name" value="<?= htmlspecialchars($product['name']) ?>" class="<?= isset($errors['name']) ? 'input-error' : '' ?>">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="text" name="sku" placeholder="SKU-001" value="<?= htmlspecialchars($product['sku']) ?>" class="<?= isset($errors['sku']) ? 'input-error' : '' ?>">
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <input type="number" name="price" placeholder="0.00" step="0.01" min="0" value="<?= htmlspecialchars($product['price']) ?>" class="<?= isset($errors['price']) ? 'input-error' : '' ?>">
        ...
        <div class="form-group">
          ...
          <input type="number" name="stock" placeholder="0" min="0" value="<?= htmlspecialchars($product['stock']) ?>" class="<?= isset($errors['stock']) ? 'input-error' : '' ?>">
      ...
      <div class="form-group">
        ...
        <input type="text" name="image_url" id="imageUrl" placeholder="https://..." value="<?= htmlspecialchars($product['image_url']) ?>" oninput="previewImage(this.value)">
      ...
      <div class="btn-row">
        <a href="index.php" class="btn-discard">Discard</a>
